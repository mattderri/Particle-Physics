...
\end{multicols}
\endbox
The two $K^0$ states have different values of CP, respectively CP=+1 and CP=$-$1. The first decays into $2\pi$ and the second into $3\pi$.
\[
K_1^0\to(2\pi) \quad K_2^0\to(3\pi)
\]
The $3\pi$ state is near the kinematic threshold
...
\end{aligned}
\]
The sign of the charged lepton flags the strangeness of the $K^0/\overline{K}^0$. The experimental measure regards the charge asymmetry $\delta=R(K^0)-R(\overline{K}^0)$, i.e. the difference between positive charged and negative charged leptons, which is directly correlated to the oscillations. The results are in good agreement with the expectations except for the tail, which is connected to CP violation.
...
CP conservation implies that $K_S^0\to(2\pi)$ and $K_L^0\to(3\pi)$. However, if we observe some decays $K_L^0\to2\pi$ or $K_S^0\to3\pi$ with small but non-zero branching ratio, it would be an experimental evidence of the non-conservation of CP. Since a small amount of $K_S^0\to3\pi$ is not observable due to background, the key observation is $K_L^0\to2\pi$.\\
...
\begin{tikzpicture}
  \begin{feynman}
    \vertex (a);
    \vertex [above=0.1cm of a] () {$V_{cd}$};
    \vertex [below=of a] (b);
    \vertex [below=0.1cm of b] () {$V_{cs}^*$};
    \vertex [right=of b] (c);
    \vertex [below=0.1cm of c] () {$V_{td}$};
    \vertex [right=of a] (d);
    \vertex [above=0.1cm of d] () {$V_{ts}^*$};
    \vertex [left=of a] (i1) {$d$};
    \vertex [left=of b] (i2) {$\overline{s}$};
    \vertex [right=of d] (f1) {$s$};
    \vertex [right=of c] (f2) {$\overline{d}$};

    \diagram* {
    (a) -- [photon, edge label'=$W^-$] (d),
    (b) -- [photon, edge label=$W^+$] (c),
    (a) -- [fermion, edge label=$c$] (b),
    ...
    \draw [decoration={brace}, decorate] 
    (i2.south west) -- (i1.north west) node [below left=0.75cm and 0.1cm] {$K^0$};
    \draw [decoration={brace}, decorate] 
    (f1.north east) -- (f2.south east) node [above right=0.75cm and 0.1cm] {$\overline{K}^0$};
  \end{feynman}
\end{tikzpicture}    
\end{center}
\end{minipage}\hfill
...
\begin{tikzpicture}
  \begin{feynman}
    \vertex (a);
    \vertex [above=0.1cm of a] () {$V_{cs}$};
    \vertex [below=of a] (b);
    \vertex [below=0.1cm of b] () {$V_{cd}^*$};
    \vertex [right=of b] (c);
    \vertex [below=0.1cm of c] () {$V_{ts}$};
    \vertex [right=of a] (d);
    \vertex [above=0.1cm of d] () {$V_{td}^*$};
    \vertex [left=of a] (i1) {$s$};
    \vertex [left=of b] (i2) {$\overline{d}$};
    \vertex [right=of d] (f1) {$d$};
    \vertex [right=of c] (f2) {$\overline{s}$};
    ...
    \draw [decoration={brace}, decorate] 
    (i2.south west) -- (i1.north west) node [below left=0.65cm and 0.1cm] {$\overline{K}^0$};
    \draw [decoration={brace}, decorate] 
    (f1.north east) -- (f2.south east) node [above right=0.75cm and 0.1cm] {$K^0$};
  \end{feynman}
\end{tikzpicture}    
\end{center}
\end{minipage}\\
...
The importance of the phenomenon depends on the value of the CKM matrix, i.e. by the quark mixing.
\subsection{$\nu$ Oscillations}
